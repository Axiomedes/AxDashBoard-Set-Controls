VERSION 5.00
Begin VB.UserControl AxDashSmallLabel 
   Appearance      =   0  'Flat
   AutoRedraw      =   -1  'True
   BackColor       =   &H008D4214&
   ClientHeight    =   1125
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   282
   ToolboxBitmap   =   "AxDashSmallLabel.ctx":0000
   ...
   End
End
Attribute VB_Name = "AxDashSmallLabel"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = True
Attribute VB_PredeclaredId = False
Attribute VB_Exposed = True
'-UC-VB6-----------------------------
'UC Name  : AxDashSmallLabel
'Version  : 0.01
...
Private m_Caption3IconFont      As StdFont
Private m_Caption3Icon  As Long
...
With UserControl
  ...
  REC.Left = 1:     REC.Top = (.ScaleHeight / 5)
  REC.Width = .ScaleWidth - 2
  REC.Height = ((.ScaleHeight / 5) * 4) - 1

  Select Case m_IconBox
    Case ebLeft
      BOX.Left = (.ScaleWidth / 8) * nScale
      BOX.Top = 1 * nScale
      BOX.Width = (.ScaleWidth / 8) * 2 * nScale
      BOX.Height = (.ScaleWidth / 8) * 2 * nScale
    Case ebRight
      BOX.Left = .ScaleWidth - ((.ScaleWidth / 8) * 3) * nScale
      BOX.Top = 1 * nScale
      BOX.Width = (.ScaleWidth / 8) * 2 * nScale
      BOX.Height = (.ScaleWidth / 8) * 2 * nScale
  End Select
  ...
  mIcon.Left = mBorder + 1 * nScale
  mIcon.Top = REC.Top + ((REC.Height - lBorder) / 3) * 2 * nScale
  mIcon.Width = (REC.Height - lBorder) / 3 * nScale
  mIcon.Height = (REC.Height - lBorder) / 3 * nScale
  
  stREC.Left = IIf(m_IconBox = ebLeft, (BOX.Left + BOX.Width + mBorder), 1) * nScale
  stREC.Top = REC.Top + mBorder * nScale
  stREC.Width = .ScaleWidth - ((.ScaleWidth / 8) + BOX.Width + mBorder + 2) * nScale
  stREC.Height = (REC.Height - lBorder) / 3 * nScale
  
  stREC2.Left = IIf(m_IconBox = ebLeft, stREC.Left, 1) * nScale
  stREC2.Top = REC.Top + ((REC.Height - lBorder) / 3) * nScale
  stREC2.Width = stREC.Width * nScale
  stREC2.Height = (REC.Height - lBorder) / 3 * nScale
  
  
  stREC3.Left = (mIcon.Left + mIcon.Width) * nScale
  stREC3.Top = REC.Top + ((REC.Height - lBorder) / 3) * 2 * nScale
  stREC3.Width = .ScaleWidth - mIcon.Width - lBorder * nScale
  stREC3.Height = (REC.Height - lBorder) / 3 * nScale
...
Effect:
  ...
  DrawCaption hGraphics, m_Caption3, m_Font3, stREC3, m_ForeColor3, CLng(m_Opacity), 0, m_Caption3AlignH, eMiddle, False
...
' '---------------
  ...
  ThisColor = GetPixel(.hdc, mIcon.Left + 5, mIcon.Top + 5)
  mIconColor = InvColor(ThisColor)
  DrawCaption hGraphics, Caption3Icon, Caption3IconFont, mIcon, mIconColor, CLng(100), 0, eCenter, eMiddle, True
  'DrawLine 3°Caption
...
cl_hWnd = UserControl.ContainerHwnd
  ...
  Set m_Caption3IconFont = UserControl.Font
...
With PropBag
  ...
  Set m_Caption3IconFont = .ReadProperty("Caption3IconFont", UserControl.Font)
  m_Caption3Icon = .ReadProperty("Caption3Icon", "&H0")
...
With PropBag
  ...
  Call .WriteProperty("Caption3IconFont", m_Caption3IconFont)
  Call .WriteProperty("Caption3Icon", m_Caption3Icon, 0)
...
Public Property Get Caption3Icon() As String
    Caption3Icon = "&H" & Hex(m_Caption3Icon)
End Property

Public Property Let Caption3Icon(ByVal New_IconCharCode As String)
    New_IconCharCode = UCase(Replace(New_IconCharCode, Space(1), vbNullString))
    New_IconCharCode = UCase(Replace(New_IconCharCode, "U+", "&H"))
    If Not VBA.Left$(New_IconCharCode, 2) = "&H" And Not IsNumeric(New_IconCharCode) Then
        m_Caption3Icon = "&H" & New_IconCharCode
    Else
        m_Caption3Icon = New_IconCharCode
    End If
    PropertyChanged "Caption3Icon"
    Refresh
End Property

Public Property Get Caption3IconFont() As StdFont
    Set Caption3IconFont = m_Caption3IconFont
End Property

Public Property Set Caption3IconFont(New_Font As StdFont)
