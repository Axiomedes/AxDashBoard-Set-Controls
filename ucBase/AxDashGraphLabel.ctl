VERSION 5.00
Begin VB.UserControl AxDGraphLabel 
   Appearance      =   0  'Flat
   AutoRedraw      =   -1  'True
   BackColor       =   &H008D4214&
   ClientHeight    =   1125
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   282
   ToolboxBitmap   =   "AxDashGraphLabel.ctx":0000
   ...
   End
End
Attribute VB_Name = "AxDGraphLabel"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = True
Attribute VB_PredeclaredId = False
Attribute VB_Exposed = True
'-UC-VB6-----------------------------
'UC Name  : AxDGraphLabel
'Version  : 0.01
...
With UserControl
  ...
  strSeccion = (.ScaleHeight / 3) * 2
  
  stREC.Left = mBorder + 3 * nScale
  stREC.Top = .ScaleHeight - (strSeccion + mBorder) * nScale
  stREC.Width = .ScaleWidth - (lBorder + 3) * nScale
  stREC.Height = strSeccion / 3 * nScale
  
  stREC2.Left = mBorder + 3 * nScale
  stREC2.Top = .ScaleHeight - mBorder - ((strSeccion / 3) * 2) * nScale
  stREC2.Width = .ScaleWidth - (lBorder + 3) * nScale
  stREC2.Height = strSeccion / 3 * nScale
  
  stREC3.Left = mBorder + 3 * nScale
  stREC3.Top = .ScaleHeight - mBorder - (strSeccion / 3) * nScale
  stREC3.Width = .ScaleWidth - (lBorder + 3) * nScale
  stREC3.Height = strSeccion / 3 * nScale
  
  REC.Left = 1 * nScale:     REC.Top = 1 * nScale
  REC.Width = .ScaleWidth - 2 * nScale
  REC.Height = .ScaleHeight - 2 * nScale
          
  BOX.Left = (.ScaleWidth / 2) - ((strSeccion / 2) / 2) * nScale
  BOX.Top = IIf(lBorder = 0, 5, (lBorder * 2)) * nScale
  BOX.Width = strSeccion / 2 * nScale
  BOX.Height = strSeccion / 2 * nScale
      
  IcoBox.Left = BOX.Left - 2: IcoBox.Top = BOX.Top - 2
  IcoBox.Width = BOX.Width + 4: IcoBox.Height = BOX.Height + 4
  ...
  If m_EffectFade Then
    'Back
    gRoundRect hGraphics, REC, ARGB(m_Color1, m_Opacity), ARGB(m_Color2, m_Opacity), m_Angulo, 0, ARGB(vbWhite, 0), m_CornerCurve, m_Filled
    'IconBox
    ...
    'Graph
    DrawGraphic hGraphics, m_Graph, m_GraphMatrix, REC, ARGB(m_GraphBackColor, 60), 0, ARGB(m_GraphLineColor, 50), 1, m_GraphStyle, True, ARGB(m_GraphPointColor, 80)
    'Border
    If m_BorderWidth > 0 Then gRoundRect hGraphics, REC, ARGB(vbWhite, 0), ARGB(vbWhite, 0), 0, m_BorderWidth, ARGB(m_BorderColor, 100), m_CornerCurve, False
    'Icon
    ...
    DrawCaption hGraphics, m_Caption3, m_Font3, stREC3, m_ForeColor3, m_Opacity, 0, m_Caption3AlignH, m_Caption3AlignV, False
  Else
    'Back
    ...
    DrawCaption hGraphics, m_Caption3, m_Font3, stREC3, m_ForeColor3, 100, 0, m_Caption3AlignH, m_Caption3AlignV, False
  End If
' '---------------
  'ToolTip
  If mShowTtp Then
    ...
    DrawCaption hGraphics, sToolTip, .Font, spREC, m_ForeColor, 100, 0, eCenter, eMiddle, False
  End If
' '---------------
...
Private Sub DrawGraphic(ByVal iGraphics As Long, mShape As eGraph, GrMatrix As String, Rct As RECTL, _
...
If BackStyle = gsGradient Then
  GdipCreateLineBrushFromRectWithAngleI Rct, BackColor, ARGB(vbWhite, 40), Angulo + 90, 0, WrapModeTileFlipXY, hBrush
Else
...
Select Case mShape
    ...
    If mShape = egRectLine Then
      'Rect Graph
      For I = 0 To p - 1
          GdipAddPathLineI mPath, iPts(I).X, iPts(I).Y, iPts(I + 1).X, iPts(I + 1).Y
      Next I
      
    Else
      'Curved Graph
      GdipAddPathCurve mPath, iPts(0), p + 1
    End If
    ...
    ReDim gRec(p) As RECTL
      ...
      For I = 0 To p
          GdipDrawRectangleI iGraphics, hPen, iPts(I).X, iPts(I).Y, s - (s / 8), (H - iPts(I).Y + 2)
          GdipFillRectangleI iGraphics, hBrush, iPts(I).X, iPts(I).Y, s - (s / 8), (H - iPts(I).Y + 2)
          'for Tooltips
          gRec(I).Left = iPts(I).X:     gRec(I).Top = iPts(I).Y
          gRec(I).Width = s - (s / 8):  gRec(I).Height = (H - iPts(I).Y + 2)
      Next I
      GoTo zEnd

End Select
      
    GdipAddPathArcI mPath, Rct.Left + Rct.Width - m_CornerCurve, (Rct.Top + Rct.Height) - m_CornerCurve, m_CornerCurve, m_CornerCurve, 0, 90
    GdipAddPathArcI mPath, Rct.Left, (Rct.Top + Rct.Height) - m_CornerCurve, m_CornerCurve, m_CornerCurve, 90, 90
...
zPoints:
  If POINTS Then
    ...
    For I = 0 To UBound(iPts)
      GdipFillEllipse iGraphics, pBrush, iPts(I).X - 3, iPts(I).Y - 3, 6, 6
    Next I
    ...
    Case egRectLine, egCurvedLine
      For I = 0 To UBound(iPts)
        If X >= iPts(I).X - 3 And X <= iPts(I).X + 3 And Y >= iPts(I).Y - 3 And Y <= iPts(I).Y + 3 Then
          m_PosX = IIf(I = UBound(iPts), X - UserControl.TextWidth(m_ToolTip(I)) - 10, X)
          m_PosY = Y
          mShowTtp = True
...
With PropBag
  ...
  m_Graph = .ReadProperty("GraphLine", 0)
  ...
  m_GraphLineColor = .ReadProperty("GraphLineColor", vbRed)
  m_GraphBackColor = .ReadProperty("GraphBackColor", vbRed)
  m_GraphPointColor = .ReadProperty("GraphPointColor", vbRed)
...
Public Property Get GraphLine() As eGraph
GraphLine = m_Graph
End Property

Public Property Let GraphLine(ByVal eG As eGraph)
  m_Graph = eG
